...
''',
    ...
    # HTML TEMPLATES
    # -------------------------------------------------------------------------
    "html_templates": Block(
        id="html_templates",
        name="HTML Templates",
        description="Server-side HTML templates with Jinja2",
        requires=[],
        provides=[
            Port("templates", "view", "HTML template rendering"),
            Port("static_files", "assets", "CSS/JS/image assets"),
        ],
        satisfies={"needs_frontend": True},
        code={
            "flask": '''
# templates.py - HTML Templates Block

import os
from flask import render_template, send_from_directory


def setup_templates(app, static_folder="static", template_folder="templates"):
...
<body>
    <header>
        <nav>
            <a href="/" class="logo">{{ app_name or 'App' }}</a>
            <div class="nav-links">
                {% block nav %}{% endblock %}
            </div>
        </nav>
    ...
    <footer>
        <p>&copy; {{ current_year or 2024 }}. Built with Blueprint.</p>
    </footer>
...
DEFAULT_CSS = """/* style.css - Generated by Blueprint */
...
def create_example_templates(template_folder="templates"):
    """Create example templates for common pages."""
    import os
    os.makedirs(template_folder, exist_ok=True)
...
<div class="card">
    <h1>Welcome</h1>
    <p>Your app is running! Edit templates in the templates/ folder.</p>
</div>
{% endblock %}"""
...
{% block content %}
<h1>{{ entity_name }}</h1>
<a href="{{ url_for('new_' + entity_name.lower()) }}" class="btn">+ New</a>

<div style="margin-top: 1rem">
{% for item in items %}
    <div class="card">
        <h3>{{ item.title or item.name or item.id }}</h3>
        <a href="{{ url_for('edit_' + entity_name.lower(), id=item.id) }}">Edit</a>
    </div>
{% else %}
    <p>No items yet.</p>
{% endfor %}
...
{% endblock %}"""
    ...
    with open(os.path.join(template_folder, "list.html"), "w") as f:
        f.write(list_template)


def create_static_files(static_folder="static"):
