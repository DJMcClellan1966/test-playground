...
RESTful API endpoints for Blog Post resource.
"""
...
# Create blueprint
blog post_bp = Blueprint("blog post", __name__, url_prefix="/api/blog post")
...
@blog post_bp.route("", methods=["GET"])
@blog post_bp.route("/", methods=["GET"])
def list_all():
    """
...
@blog post_bp.route("", methods=["POST"])
@blog post_bp.route("/", methods=["POST"])
def create():
    """
    Create a new Blog Post.
    
    Request body: JSON with Blog Post fields
    Response: Created Blog Post with ID and timestamps
    """
...
@blog post_bp.route("/<item_id>", methods=["GET"])
def get_one(item_id: str):
    ...
    if not item:
        abort(404, description=f"Blog Post with id '{item_id}' not found")
...
@blog post_bp.route("/<item_id>", methods=["PUT"])
...
@blog post_bp.route("/<item_id>", methods=["PATCH"])
...
@blog post_bp.route("/<item_id>", methods=["DELETE"])
...
@blog post_bp.route("/bulk", methods=["POST"])
def bulk_create():
    """
    Create multiple Blog Post items at once.
...
@blog post_bp.route("/bulk", methods=["DELETE"])
def bulk_delete():
    """
    Delete multiple Blog Post items.
