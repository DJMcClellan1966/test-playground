...
try:
    ...
    # Try importing from local template_evolution first
    try:
        # Try importing from local template_evolution if available
        import importlib.util
        import sys
        import os
        module_path = os.path.join(os.path.dirname(__file__), "template_evolution.py")
        if os.path.exists(module_path):
            spec = importlib.util.spec_from_file_location("template_evolution", module_path)
            template_evolution = importlib.util.module_from_spec(spec)
            sys.modules["template_evolution"] = template_evolution
            spec.loader.exec_module(template_evolution)
            TemplateEvolver = template_evolution.TemplateEvolver
            TemplateVector = template_evolution.TemplateVector
            ALL_FEATURES = template_evolution.ALL_FEATURES
            FEATURE_INDEX = template_evolution.FEATURE_INDEX
        else:
            raise ImportError("template_evolution module not found")
    except ImportError:
        # Fallback failed: could not import template_evolution from any location
        HAS_SYSTEMS = False
        TemplateEvolver = None
        TemplateVector = None
        ALL_FEATURES = []
        FEATURE_INDEX = {}
