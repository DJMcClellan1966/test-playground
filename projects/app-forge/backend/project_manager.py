    ...
    def create_project(self, app_name: str, description: str, answers: Dict[str, bool], 
       ...
       # Create structure
        ...
        metadata = {
            ...
            "answers": answers,
            "created_at": str(Path(project_path)).split("\\")[-1],  # Simple timestamp
        }
...
"""
    ...
    def init_git(self, project_path: Path, remote_url: str = None) -> bool:
        ...
        try:
            ...
            subprocess.run(["git", "config", "user.email", "appforge@local"], check=True, capture_output=True)
            subprocess.run(["git", "config", "user.name", "AppForge"], check=True, capture_output=True)
